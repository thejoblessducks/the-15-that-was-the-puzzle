...
public class Puzzle{
    ...
    //Maker Table General---
    public static Table stepToTake(Table t, int move,HashMap<String,String[]> h, int method,int n){
        //2-DFS
        ...
        if(h.containsKey(t)){
            if(method != 2)
                return null;
            int j = Integer.parseInt(h.get(t.getString())[1]);
            if(n<j){
    ...
    //Search Methods
    public static void tableDFS(Table tb){
        ...
        while(!stack.isEmpty()){
            ...
            for(int i= -2; i<3; i++){
                ...
                tb= stepToTake(t,i,h,2,tmp);
                if(!(tb == null) && !in_stack.containsKey(tb.getString())){
                    //if null, is invalid movement, or not solvable or already existing
                    ts=putPath(h.get(t.getString())[0], tb.getLastMove());
                    h.put(tb.getString(),makePath(ts,h.get(t.getString())[1]));
                    if(h.get(t.getString())[1].equals("79")) continue;
                    in_stack.put(tb.getString(),null);
                    stack.push(tb);
                    tb=null;
                }
